...
public static class PageMapper
{
    ...
    public static string MapAdminPage(int userId)
    {
        ...
        var missionsInOneHtml = MapMissionsForHomePage(missionTemplate, userId);
        ...
        var screenshotCardsInOneHtml = MapScreenshotCardsForHomePage(screenshotCardTemplate, userId);

        // Mapping video cards
        var videoCardTemplate = StaticFilesHandler.FetchStaticFile("components/adminPageVideo.html");
        var videoCardsInOneHtml = MapVideoCardsForHomePage(videoCardTemplate, userId);
    ...
    private static string MapMissionsForHomePage(string missionTemplate, int userId = 0)
    {
        // Mapping missions
        var missionDao = new MissionDao();
        var missionsList = userId != 0
            ? missionDao.GetAllMissionsByUserId(userId)
    ...
    private static string MapScreenshotCardsForHomePage(string screenshotCardTemplate, int userId = 0)
    {
        // Mapping screenshot cards
        var screenshotCardDao = new ScreenshotCardDao();
        var screenshotCardsList = userId != 0
            ? screenshotCardDao.GetAllScreenshotCardsByUserId(userId)
            : screenshotCardDao.GetAllScreenshotCards();
    ...
    private static string MapVideoCardsForHomePage(string videoCardTemplate, int userId = 0)
    {
        // Mapping video cards
        var videoCardDao = new VideoCardDao();
        var videoCardsList = userId != 0
            ? videoCardDao.GetAllVideoCardsByUserId(userId)
            : videoCardDao.GetAllVideoCards();
